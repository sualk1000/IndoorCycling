...
public class GarminConnect {
  ...
  public String uploadFitFile(File fitFile) {
    ...
    try {
      ...
      if (js_upload.getJSONObject("detailedImportResult").getJSONArray("failures").length() != 0) {
        JSONObject js_failure =  js_upload.getJSONObject("detailedImportResult").getJSONArray("failures").getJSONObject(0);
        if(js_failure.has("messages") && js_failure.getJSONArray("messages").length() != 0)
        {
          JSONObject message  = js_failure.getJSONArray("messages").getJSONObject(0);
          throw new Exception("Upload error " + message.getString("content"));
  ...
  public String uploadFitFile(String fitFilePath) {
